...
func parseTokenWithClaims(tokenStr, salt string) (*generates.JWTAccessClaims, error) {
	// Parse and verify jwt access token
	token, err := jwt.ParseWithClaims(tokenStr, &generates.JWTAccessClaims{}, func(token *jwt.Token) (interface{}, error) {
		if _, ok := token.Method.(*jwt.SigningMethodHMAC); !ok {
			return nil, fmt.Errorf("unexpected signing method")
		}
		return []byte(salt), nil
...
// ---------------------------------------- Middleware Oauth Token 驗證 ----------------------------------------------

func ParseOauthToken(tokenStr string) (*generates.JWTAccessClaims, error) {
	salt := os.Getenv("OAUTH_SALT")
	return parseTokenWithClaims(tokenStr, salt)
}
